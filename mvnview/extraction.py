...
class MVNX:
    def __init__(self, path, orientation=None, position=None, velocity=None, \
                 ...
                 jointAngleErgoXZY=None, centerOfMass=None, mapping=None, sensors=None, segments=None, joints=None, \
    ...
    def parse_modality(self, modality):
        ...
        for frame in tqdm(frames[3:]):
            child = frame.find(self.ns + modality)
    ...
    def save_to_HDF5(self, filepath):
        ...
        with h5py.File(f'{filepath}.hdf5', 'w') as f:
            ...
            jointAngleErgo = f.create_dataset("jointAngleErgo", data=self.jointAngleErgo)
            jointAngleErgoXZY = f.create_dataset("jointAngleErgoXZY", data=self.jointAngleErgoXZY)
            velocity = f.create_dataset("velocity", data=self.velocity)
            acceleration = f.create_dataset("acceleration", data=self.acceleration)
